...
samples_all = pd.read_excel(
    "../data/metadata/samples_all.xlsx",
    usecols="B,C,E,F,H:K",
    sheet_name="Overview donors",
)

samples_all.drop(
    [13, 40, 41, 82, 83, 98], inplace=True
)  # remove empty rows depending on the excel format!
...
samples_all.rename(
    columns={"Population": "subset", "Cell number": "Cell Number"}, inplace=True
)
...
metadata_gsid = {
    "sampleID": sampleID,
    "read1_file_name": read1_file_name,
    "read2_file_name": read2_file_name,
}
